...
//设置连接rabbitmq主机
connectionFactory.setHost("39.107.142.16");
//设置连接端口号
...
'参数1':用来声明通道对应的队列
'参数2':用来指定是否持久化队列-->
    true：持久化-->当我们重启rabbitmq-server之后，rabbitmq的队列依然存在
    false：不持久化-->当我们重启rabbitmq-server之后，rabbitmq的队列不存在
'参数3':用来指定是否独占队列-->
    true：独占队列
    fasle：不独占队列
'参数4':用来指定是否自动删除队列-->
    true：自动删除-->消息被消费完并且消费者与消息队列断开连接之后，队列就会自动删除
    fasle：不自动删除
'参数5':对队列的额外配置
```
...
<img src="C:\Users\86159\AppData\Roaming\Typora\typora-user-images\image-20200725172756729.png" style="zoom:50%;" />

<img src="C:\Users\86159\AppData\Roaming\Typora\typora-user-images\image-20200725172846780.png" style="zoom:50%;" />

> `总结:默认情况下(轮询)，RabbitMQ将按顺序将每个消息发送给下一个使用者。平均而言，每个消费者都会收到相同数量的消息。这种分发消息的方式称为循环。`

##### 5.消息自动确认机制(“能者多劳”)

==出现问题：==

实际场景1：在消费者1睡眠1秒钟，虽然该业务执行慢，但是依然拿到平均的消息

实际场景2：在消费者运行过程中，突然宕机，导致确认拿到了消息，但是消息实际上并没有被业务获取，导致消息丢失
...
public class Provider {
    public static void main(String[] args) throws IOException {
        ...
        String key = "warning";
        //发布消息
        channel.basicPublish(exchange,key,null,("指定的route key"+key+"的消息").getBytes());
        RabbitMQUtils.closeConnectionAndChannel(connection,channel);
    }
}
```
...
public class Provider {
    public static void main(String[] args) throws IOException {
        ...
        //动态路由key
        String routekey = "user.find.getAll";
        //发布消息
